...
class RiskLearn:
        ...
        while not done:
            ...
            if cp == 1:
                ...
                if random.random() < self.epsilon:
                    ...
                    qv   = self.q_model(self.env.perspective(state,1)[None]).numpy()[0]
                    mask = np.full(self.N, -1e9)
                    mask[valid] = qv[valid]
                    idx = int(mask.argmax())
            ...
            next_state, scaled_r, done, info = self.env.step(action)
            ...
            # store & learn if P1
            if cp == 1:
                self.replay.append((state.copy(), idx, scaled_r, next_state.copy(), done))
    ...
    def train(self, episodes=256, show=False):
        ...
        for ep in tqdm(range(1, episodes+1)):
            ...
            # print summary
            print(f"Episode : {ep}/{episodes} | eps : {self.epsilon:.3f} | reward : {raw_r:+.3f} | steps : {s}")

            # force end‐phase and render final state + raw reward once
            self.env.phase = 3
            print("** NOW PHASE: End **")
            self.env.render()
...
if __name__ == "__main__":
    ...
    learner.train(episodes=64, show=False)
